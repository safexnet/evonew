...
func ServeWs(w http.ResponseWriter, r *http.Request, instanceId string, producer *websocketProducer) {
	logger.LogInfo("Starting WebSocket connection upgrade")
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		logger.LogError("Error upgrading websocket connection: %v", err)
		return
	}

	logger.LogInfo("WebSocket connection successfully established")

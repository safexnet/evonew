...
```bash
git clone https://github.com/safexnet/evogo.git
cd evogo

# Build Docker image
docker build -t evoapicloud/evolution-go:latest .

# Run with Docker Compose (Background mode)
docker compose -f docker/examples/docker-compose.yml up -d

# Or run with live terminal logs

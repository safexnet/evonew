...
func setupRouter(db *gorm.DB, authDB *sql.DB, sqliteDB *sql.DB, config *config.Config, conn *amqp.Connection, exPath string, runtimeCtx *core.RuntimeContext) *gin.Engine {
	...
	r.GET("/ws", func(c *gin.Context) {
		...
		if token != config.GlobalApiKey {
			logger.LogError("Invalid token: %s", token)
...
func initPostgresAuthDB(config *config.Config) (*sql.DB, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error connecting to AUTH PostgreSQL database: %v", err)
	}
	...
	err = db.Ping()
	if err != nil {
		return nil, fmt.Errorf("error pinging AUTH PostgreSQL database: %v", err)
	}

	logger.LogInfo("Connected to AUTH PostgreSQL database with configured pool")
	return db, nil
}
...
func main() {
	...
	go func() {
		logger.LogInfo("Starting server on port %s", os.Getenv("SERVER_PORT"))
